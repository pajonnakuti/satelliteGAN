"""
88_spatial_chronos_only_deterministic.py - Deterministic Chronos-only spatial run
==============================================================================
Same pipeline as 86_spatial_chronos_only.py with deterministic controls and
sampling parameters exposed via env vars. Original 86 remains untouched.

USAGE:
  DETERMINISTIC=1 SEED=123 python 88_spatial_chronos_only_deterministic.py
  NUM_SAMPLES=1 TEMPERATURE=0.0 TOP_P=1.0 python 88_spatial_chronos_only_deterministic.py
"""
import os
...
try:
    ...
    raise RuntimeError("pip install chronos-forecasting")

import torch

SCRIPT_NAME = Path(__file__).name if "__file__" in globals() else "88_spatial_chronos_only_deterministic.py"
...
DETERMINISTIC = os.environ.get("DETERMINISTIC", "1").strip().lower() not in ("0", "false", "no")
...
print("=" * 80)
print(f"{SCRIPT_NAME} | Chronos-only deterministic")
print(
    f"Calibrate={CALIBRATE} | Point-only={POINT_ONLY} | Adaptive={not DISABLE_ADAPTIVE} | "
    f"PostGain={POST_GAIN} | Deterministic={DETERMINISTIC} | Seed={SEED}"
)
print(f"Sampling: samples={NUM_SAMPLES} temp={TEMPERATURE} top_p={TOP_P}")
print("=" * 80)
...
print(f"  Chronos bias: {np.round(bias, 4)}")
...
for t in range(pred_start_abs - SEQ_LEN, pred_end_abs - HORIZON + 1):
    ...
    point_pred = chronos_forecast(ctx)
...
if POINT_ONLY:
    ...
    def draw_spatial_png(blocks_pair, month_label, part_label, m_rmse, out_path):
        ...
        plt.suptitle(
            f"{SCRIPT_NAME} | {month_label} {part_label} | Spatial Chronos-only\n"
...
ax_main.plot(x, avg_series, color="crimson", lw=2.8, zorder=7, alpha=0.95,
             label=f"Chronos-only RMSE={overall_rmse:.4f} R2={r2_val:.4f} gates={gates_pass}/5")
...
ax_main.set_title(
    f"{SCRIPT_NAME} | Chronos-only | {PRED_START_DATE.date()} -> {PRED_END_DATE.date()}\n"
...
ax.plot(xf, slope * xf + intercept, "r-", lw=2.5, alpha=0.85, label=f"Chronos slope={slope:.3f}")
if convlstm_pred is not None:
...
ax.set_title(f"{SCRIPT_NAME} - Chronos correlation | RMSE={overall_rmse:.4f} slope={slope:.4f} gates={gates_pass}/5",
             fontsize=13, fontweight="bold")
handles += [mpatches.Patch(color="k", label="y=x"),
            mpatches.Patch(color="r", label=f"Chronos slope={slope:.3f}")]
if convlstm_pred is not None:
...
ax_bar.set_title("Monthly RMSE: Chronos vs ConvLSTM", fontsize=11, fontweight="bold")
...
for i in range(len(table_data)):
    if "Chronos" in table_data[i][0]:
        for j in range(len(table_data[i])):
            tbl[i+1, j].set_facecolor("#FFE0E0")
            tbl[i+1, j].get_text().set_fontweight("bold")

plt.suptitle(f"{SCRIPT_NAME} | Spatial Chronos-only vs References | {PRED_START_DATE.date()} -> {PRED_END_DATE.date()}",
...
print("\n" + "=" * 80)
print("CHRONOS-ONLY SPATIAL RUN COMPLETE")
